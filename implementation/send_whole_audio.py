...
def on_wakeword_detected():
    ...
    realtime_event_operations.insert_conversation_item(
        ws_interface, "system", text=f"Current time settings: {json.dumps(datetime.get_current_time_settings())}")
...
audio_recorder = AudioRecorder(
    on_speech_finished_callback=lambda audio_bytes: (
        realtime_event_operations.insert_conversation_item(ws_interface, "user", audio_bytes=audio_bytes),
